...
export function init() {
  ...
  if (!storage.getItem("playerActive")) {
    storage.setItem(
      "playerActive",
      Object.fromEntries(
...
export function players() {
  Object.keys(storage.getItem("playerActive")).sort();
}

export function isActivePlayer(player) {
  return storage.getItem("playerActive")[player];
}

export function activatePlayer(player, active) {
  const playerActive = storage.getItem("playerActive");
  playerActive[player] = active;
  storage.setItem("playerActive", playerActive);
}
