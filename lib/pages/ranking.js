...
import * as tfr from "../tf.ranking.js";
...
// Computes a list of players with the number of events they have participated in.
function participations(matches) {
  const playerEvents = {};

  for (const match of matches) {
    for (const player of util.matchPlayers(match)) {
      if (!playerEvents[player]) {
        playerEvents[player] = {};
      }
      playerEvents[player][match.event] = true;
    }
  }

  return Object.entries(playerEvents)
    .map(([player, events]) => ({
      player,
...
export const resetRanking = () => {
  ...
  ranking.innerHTML = "";
  rankingFrom(selectedMatches()).then((rk) =>
    ranking.appendChild(rankingTable(rk))
  );
  // console.log(participations(selectedMatches()));
};
...
async function rankingFrom(matches) {
  matches = matches.filter((match) => match.games.length > 0);
  const perfs = await tfr.rankingFrom(matches);
  perfs.forEach(({ player, performance }) => {
    currentPlayerPerformances[player] = performance;
  });
  return perfs
...
function pointCounts(players, tpGames) {
  const result = {};
  ...
  for (const game of tpGames) {
    const playedPoints = game.points.home + game.points.away;
    result[game.home] += playedPoints;
    result[game.away] += playedPoints;
  }
...
function playerPerformances(players, tpGames, pointCounts) {
  ...
  while (count++ < 10000 && error > bound)
    error = adjustPerformances(players, tpGames, pointCounts);

  console.log(count, "iterations");

  const perfValues = Object.values(currentPlayerPerformances);
  if (perfValues.length > 0) {
    const meanPerf = geoMean(perfValues);
    for (const player of players) {
...
function adjustPerformances(players, tpGames, pointCounts) {
  const corrections = {};
  ...
  for (const game of tpGames) {
    const [homeCorrection, awayCorrection] = calcCorrections(
      game.home,
      game.away,
      game.points,
      pointCounts
    ...
    corrections[game.home].push({ value: homeCorrection, weight });
    corrections[game.away].push({ value: awayCorrection, weight });
  }
...
const calcCorrections = (home, away, points, counts) => {
  const diversion =
  ...
  const countSum = homeCount + awayCount;
  return [
    Math.pow(diversion, awayCount / countSum),
    Math.pow(diversion, -homeCount / countSum),
  ];
};
...
function resetPlayerResults(player) {
  ...
  const matches = selectedMatches().filter((match) => match.games.length > 0);
  const players = util.allPlayers(matches);
  const tpGames = matchesAsTwoPlayerGames(matches);
  const counts = pointCounts(players, tpGames);
  playerResultsElems(player, counts).forEach((elem) =>
    playerResults.appendChild(elem)
  );
}

const playerResultsElems = (selectedPlayer, counts) => [
