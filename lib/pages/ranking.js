...
export const page = () => [
  ...
  html.elem("main", { class: "ranking" }, [
    ...
    html.elem("div", { id: "todays-players" }, [todaysPlayersSelector()]),
    nextRoundSection(),
  ]),
];
...
function havePlayedToday(players) {
  ...
  return (
    state
      ...
      .filter((match) => names === util.matchPlayers(match).sort().join())
      .length > 0
  );
}

function atLeastNhavePlayedToday(n, players) {
  return (
    state
      .matches(state.selectedEvent())
      .filter(
        (match) =>
          util.intersection(players, util.matchPlayers(match)).length >= n
      ).length > 0
  );
}
...
function searchCombinations(ixs, players) {
  ...
  if (atLeastNhavePlayedToday(ixs.length === 1 ? 2 : 3, [fst, ...others])) {
